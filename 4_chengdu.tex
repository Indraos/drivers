...
A main challenge in our analysis is to recover the number of reward units corresponding to an RMB.
...
\[
	e_t = s_t(b_t + l_t L + d_t D).
\]
Our main goal from now is to calibrate the model to estimate the 
\begin{table}
\begin{tabular}{cccc}
%TODO: Fill out table.
\end{tabular}
...
We now run our regression to recover the amoung of money a reward unit comes with. 
