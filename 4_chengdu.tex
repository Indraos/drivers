...
\epigraph{Compared to dispatching where the vehicle transitions are deter- mined by the origins and destinations ofthe outstanding orders, the repositioning operation is a proactive measure that has the freedom to make recommendations of any feasible destinations inside the city. On the flip side, it becomes a much complicated problem when the goal is to improve the average driver income of a large fleet for an extended period of time (e.g., more than 8 hours per day), which often requires some form of coordination among the vehicles to avoid causing undesirable competitions, e.g., crowding too many vehicles into a single high-demand location.}{\cite{Tang2021}}
