...
We fine-tuned a natural language model BERT \cite{Devlin2019} trained on the Indonesian Wikipedia. We fine-tune the model, i.e. train the last layers of the neural network.

Prominent predictors for  are fok (focus) and pekerjaan (profession), which hint to two areas of explanation: 
\begin{enumerate}
	\item GoJek Drivers might prefer their platform because of the cognitive load of learning another platform. This corresponds to \emph{focus} 
	\item GoJek drivers view their work more as a regular employment than Grab drivers, and therefore, for example, do not multi-home.
\end{enumerate}
The former reason can be seen as a reason to neither multi-home nor switch. The latter reason can be seen as an anti-multi-homing reason. We leave further analysis for further work.
\begin{figure}
\centering
\includegraphics[width=\linewidth]{shap.png}
\caption{Sum of Feature Importance for Decision to label as GoJek (SHAP scores)}	
\end{figure}
\section{Discussion}
