...
interests. While the commitment assumption may be problematic in many settings, it provides a useful benchmark.--\cite
\end{quote}
In this section, we study pure information design for platform drivers. We refer to it as \emph{pure} as we abstract from its interaction with pay. 
\section{Model}
In our stylized model, two agents simultaneously decide to drive to a part of a city or to do something else. Before each of the drivers makes her decision on whether to drive, the system can give them a message $m$ on whether there is demand on the platform.
